...
class Registry {
	/**
	 * The array of page-like types.
	 *
	 * @var array
	 */
	public $page_like_types = array();
	...
	/**
	 * Opt the page-like types into the PRC Platform post-publish-pipeline.
	 *
	 * @param array $post_types The post types.
	 * @return array The post types.
	 */
	public function opt_page_like_types_into_pipeline( $post_types ) {
		foreach ( $this->page_like_types as $slug => $args ) {
	...
	/**
	 * Register a page-like type.
	 *
	 * @param array $registration_args The registration arguments for the page-like type.
	 * @param array $post_type_args The post type arguments for the page-like type.
	 * @param array $taxonomies The taxonomies for the page-like type.
	 */
	public function register( $registration_args = array(), $post_type_args = array(), $taxonomies = array() ) {
		$registration_args = wp_parse_args(
			...
			array(
				'slug'        => 'page-like-type',
				'singular'    => 'Page Like Type',
				'plural'      => 'Page Like Types',
				'description' => 'Description of the page like type post type',
				'pub_listing' => false,
			)
		);
		...
		$this->page_like_types[ $registration_args['slug'] ] = $this->construct_args( $registration_args, $post_type_args, $taxonomies, $registration_args['pub_listing'] );
	}
	 ...
	 */
	public function add_notes_support( $post_type = 'page' ) {
	...
	/**
	 * Get the page-like types.
	 *
	 * @return array The page-like types.
	 */
	public function get_page_like_types() {
		return $this->page_like_types;
	}

	/**
	 * Get the page-like post-type slugs.
	 *
	 * @return array The page-like post-type slugs.
	 */
	public function get_page_like_types_post_types() {
		$post_types = array_keys( $this->page_like_types );
		return $post_types;
	}

	/**
	 * Construct labels for a page-like type.
	 *
	 * @param string $singular The singular name of the page-like type.
	 * @param string $plural The plural name of the page-like type.
	 * @return array The labels for the page-like type.
	 */
	public function construct_labels( $singular = 'Page Like Content Type', $plural = 'Page Like Content Types' ) {
		return array(
			'name'                  => $plural,
	...
	/**
	 * Construct arguments for a page-like type.
	 *
	 * @param array $registration_args The registration arguments for the page-like type.
	 * @param array $post_type_args The post type arguments for the page-like type.
	 * @param array $taxonomies The taxonomies for the page-like type.
	 * @param bool  $pub_listing Whether this post type should be included in publication listings.
	 * @return array The arguments for the page-like type.
	 */
	...
	/**
	 * Register the page-like content types.
	 *
	 * @hook init
	 */
	public function register_page_like_types() {
		foreach ( $this->page_like_types as $slug => $args ) {
	...
	/**
	 * Whenever a page-like content type is updated it should have the matching format enforced.
	 *
	 ...
	 * @return void
	 */
	public function enforce_page_like_type_format( $post ) {
		if ( in_array( $post->post_type, $this->get_page_like_types_post_types() ) ) {

			$format              = wp_get_object_terms( $post->ID, 'formats' );
			$has_matching_format = array_filter(
				$format,
				function ( $term ) {
					return in_array( $term->slug, $this->get_page_like_types_post_types() );
				}
			);
	...
	/**
	 * Set fact-sheet post type to use Report schema type.
	 *
	 ...
	 */
	public function set_fact_sheet_schema_type( $schema_type, $post_type, $post_id ) {
		if ( 'fact-sheet' === $post_type ) {
			return 'Report';
		}
		return $schema_type;
	}
